...
11. **Party Whip merge (v1.1.0, 13 June 2026).** Useful features from the earlier "Party Whip Operations" single-file prototype were wired into the modular system (the prototype's weaker engines — `\n→<br>` markdown, naive CSV split, unaudited intake — were discarded in favour of the existing ones): **archetype taxonomy** (Organizer/Academic/Journalist/General; free-text roles still valid, the four canonical ones get icons + card accents), **War Room** cohort filter + copy-emails mobilisation, **tactical mission templates** (3 built-ins + user-saved packs in `state.templates`; challenge gates downgrade to trust inside templates so pass-answers never travel), **live due-date parse preview**, Exocortex **editor toolbar + concept graph** (notes as nodes, wiki-links as edges), **System Status** panel (state footprint, entity counts, factory reset, full backup), header **Cowra clock + Substack link**. Asset version bumped to `?v=1.1.0`.
12. **Frontline field merge (v1.1.1, 13 June 2026).** Useful pieces of the "Civitas Frontline v1.0" prototype wired into the real PWA (its simulated camera, plaintext dispatches and localStorage state were discarded — ours are real): **live shift ticker** (`ON DUTY hh:mm:ss`, pulsing label), **paste-import briefings** (packs arriving via Signal/SMS load without touching the file system), **copy-sealed-JSON dispatch transport** (the AES-GCM packet is ciphertext, so any channel is safe), **copy-rebuttal buttons** on Pocket MMT cards (for online arguments), **Card 09 "How Do We Pay For It?"**, and the **Field Simulation** dev tool — injects a 3-completion burst at 250ms gaps so HQ's Airlock auditor can be demonstrated/trained end-to-end (verified: trips BATCH, DENSITY, UNKNOWN_AGENT, UNKNOWN_MISSION). Service worker cache bumped to `civitas-frontline-v2`.
13. **Same-origin caveat:** if Commander and Frontline are opened in the same browser on the same origin, Frontline's Burn Protocol (`localStorage.clear()`) also wipes the Commander's *mirror* — the vault file on disk is untouched, which is precisely why the file is canonical. In real deployments Frontline runs on the agent's phone, so the situation only arises in testing.
14. **Campaign cards merge (v1.1.2, 13 June 2026).** The "Fightback 2.0 — Campaign Cards" 17-card policy platform was integrated as: (a) `campaign/cards.html` — a dedicated print-ready page served from the Commander's localhost, with dark terminal screen theme and clean white print layout (`@media print`, A4 landscape, dashed fold-and-cut borders, `Ctrl+P` shortcut); (b) a fifth Frontline nav tab (**PLATFORM**, ⚑) with all 17 cards as accordion entries with ⧉ COPY NEW WAY per card; (c) a **CARDS** quick-link in the Commander header. The two-card-type system is now complete: Pocket MMT answers "why does the money exist?" (epistemological/monetary); Platform answers "what will you do with it?" (substantive policy). Frontline service worker bumped to `civitas-frontline-v3`.
